...
## 
##-------------------------------------------------------------------------
def plot_zeropoints(DM, cfg=None):
    '''
    '''
    ...
    for c,color in enumerate(DM.zero_point.keys()):
...
#         plt.title(f'{color} Zero Point = {DM.zero_point[color]:.3f} (std = {DM.zero_point_stddev[color]:.3f})')
        binsize = 0.05
        min_bin = np.round(DM.zero_point[color]/binsize)*binsize - 10*np.round(DM.zero_point_stddev[color]/binsize)*binsize
        max_bin = np.round(DM.zero_point[color]/binsize)*binsize + 10*np.round(DM.zero_point_stddev[color]/binsize)*binsize
        ...
        plt.axvline(DM.zero_point[color], color='k',
                    label=f'{color} Zero Point = {DM.zero_point[color]:.2f}')
        plt.axvline(DM.zero_point[color]-DM.zero_point_stddev[color],
                    color='k', linestyle=':')
        plt.axvline(DM.zero_point[color]+DM.zero_point_stddev[color],
                    color='k', linestyle=':')
        plt.legend(loc='best')
        ...
        plt.plot(outliers['peak_value'], outliers[f'{color}ZeroPoint'], 
                 'kx', ms=3, alpha=0.5,
                 label='5 sigma Outliers')
        plt.axhline(DM.zero_point[color], color='k', alpha=0.5,
                    label=f'{color} Zero Point = {DM.zero_point[color]:.2f}')
        plt.legend(loc='best')
        plt.xscale('log')
        plt.xlim(7e2,8e4)
    ...
    plot_file = Path(DM.raw_file_name.replace(ext, '_zp.png'))
